...
if (url.pathname === '/api/admin/terminal-password' || url.pathname === '/api/admin/terminal-password/') {
return await handleAdmin(req, res, 'terminal-password');
}
// ======== ROUTER GENERAL ========
...
.from('config')
.select('election_status, school_logo_url, school_name, terminal_password_enabled')
.eq('id', 1)
.single();
...
terminalPasswordEnabled: data.terminal_password_enabled || false
});
}
async function verifyCode(req, res) {
const supabase = getSupabase();
if (req.method !== 'POST') return res.status(405).json({ error: 'Método no permitido' });
const { access_code, terminal_password } = req.body || {};
if (!access_code || !/^\d{5}$/.test(access_code)) {
return res.status(400).json({ error: 'Código inválido (debe tener 5 dígitos)' });
}
// Verificar si la contraseña de terminal está habilitada
const { data: config, error: cfgError } = await supabase
.from('config')
.select('terminal_password_enabled, terminal_password')
.eq('id', 1)
.single();
if (cfgError) return res.status(500).json({ error: 'Error de configuración' });
// Si está habilitada, validar contraseña
if (config.terminal_password_enabled && config.terminal_password) {
if (!terminal_password || terminal_password !== config.terminal_password) {
return res.status(403).json({ error: 'Contraseña de terminal incorrecta' });
}
}
const {  student, error } = await supabase
.from('students')
...
const supabase = getSupabase();
if (req.method !== 'POST') return res.status(405).json({ error: 'Método no permitido' });
const { access_code, candidate_id, terminal_password } = req.body || {};
if (!access_code || !candidate_id) return res.status(400).json({ error: 'Datos incompletos' });
// Verificar si la contraseña de terminal está habilitada
const {  config, error: cfgError } = await supabase
.from('config')
.select('terminal_password_enabled, terminal_password')
.eq('id', 1)
.single();
...
const supabase = getSupabase();
if (req.method !== 'POST') return res.status(405).json({ error: 'Método no permitido' });
const { terminal_password } = req.body || {};
const {  config, error } = await supabase
.from('config')
.select('terminal_password_enabled, terminal_password')
.eq('id', 1)
.single();
if (error) return res.status(500).json({ error: 'Error de configuración' });
if (!config.terminal_password_enabled) {
return res.status(200).json({ valid: true, message: 'Contraseña de terminal no requerida' });
}
if (!terminal_password || terminal_password !== config.terminal_password) {
return res.status(403).json({ valid: false, error: 'Contraseña de terminal incorrecta' });
}
return res.status(200).json({ valid: true, message: 'Contraseña válida' });
}
...
const { data, error } = await supabase
.from('config')
.select('school_logo_url, school_name, terminal_password_enabled')
.eq('id', 1)
.single();
...
const {  config, error: cfgErr } = await supabase
  .from('config')
  .select('admin_code')
  .eq('id', 1)
  .single();
...
case 'terminal-password': return await handleTerminalPassword(req, res);
...
async function handleTerminalPassword(req, res) {
const supabase = getSupabase();
if (req.method === 'GET') {
const { data, error } = await supabase
.from('config')
...
.eq('id', 1)
.single();
if (error) return res.status(500).json({ error: 'Error al cargar configuración' });
return res.status(200).json({
terminalPasswordEnabled: data.terminal_password_enabled || false,
terminalPassword: data.terminal_password || ''
});
}
if (req.method === 'POST') {
...
updateData.terminal_password_enabled = enabled;
}
if (password !== undefined) {
updateData.terminal_password = password;
}
const { error } = await supabase
.from('config')
.update(updateData)
...
const supabase = getSupabase();
if (req.method !== 'POST') return res.status(405).json({ error: 'Método no permitido' });
const {  students, error: fetchError } = await supabase
.from('students')
...
const {  totalVotes } = await supabase.from('candidates').select('votes');
const sumVotes = totalVotes?.reduce((a, b) => a + (b.votes || 0), 0) || 0;
const {  byGrade } = await supabase.from('participation_by_grade').select('');
const {  results } = await supabase.from('election_results').select('');
return res.status(200).json({
general: {
...
const supabase = getSupabase();
const adminCode = req.headers['x-admin-code'];
const {  config } = await supabase.from('config').select('admin_code').eq('id', 1).single();
if (!config || adminCode !== config.admin_code) return res.status(401).json({ error: 'No autorizado' });
try {
